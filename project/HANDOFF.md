...
변경 이력:
- 2026-08-07: **v0.17.0 시설 사진 (비공개 저장소) + 회귀 버그 긴급 수정**. ① **🐞 회귀 수정 — v0.15.0에서 내가 깨뜨린 것**: 시설별 화면 분기를 넣으면서 기존 블록을 통째로 갈아끼웠는데 그 안에 있던 `facadeView`·`viewZone`·`blankView`·`infoBlock`이 함께 사라졌다. 결과로 **A·B·C·캄동을 열면 `facadeView is not defined`로 화면이 안 떴고**(하우스맨이 가장 많이 쓰는 경로), 비바 플렉스 몰에서 카테고리를 누르면 `viewZone` 없음으로 죽었다. v0.15.0·v0.16.0이 라이브였던 동안 계속 깨져 있었다. 네 함수를 복원하고 **31곳 전수 + 카테고리 진입까지 재검증**(오류 0, B 14층·C 11층·캄 17층 정상). **교훈**: 큰 블록을 통으로 교체할 때는 교체 전후의 함수 목록을 비교할 것. 이번에 미정의 호출 스캔을 검증 절차에 추가했다. ② **시설 사진**: 비공개 저장소 `places/{placeId}/` 에 두고 **팀 코드로 연결된 기기만 토큰으로 받아간다**. 공개 주소(`jykim5215.github.io/.../places/...`)로는 404 — 확인함. 기존 객실 사진(`photos/{동}/{호수}.jpg`, `Store.roomPhoto`)과 **경로를 분리**해 건물 외관 레일에 객실 내부 사진이 섞이지 않게 했다. ③ 화면: 사진이 있으면 일러스트 대신 **가로 스와이프 레일 + 인디케이터**, 탭하면 전체화면 뷰어. 없으면 일러스트 유지. **사진 추가** 버튼 — 여러 장 선택 → 긴 변 1600px JPEG(품질 .82)로 줄여 업로드. 오프라인·미연결이면 '팀 서버 연결 후'로 안내. ④ 소노벨 A·B·C·소노펠리체 외관 사진 4건을 비공개 저장소에 올려 뒀다(공식홈 수집분). API 응답 형태(list/load)가 앱 코드가 기대하는 것과 일치함을 확인.
- 2026-08-07: **v0.16.0 자주 묻는 것 + 객실 상태 보드 + 요금 계산기**. 사용자가 네 가지를 모두 추가하기로 선택(FAQ·고객 안내문 변환·상태 보드·계산기), FAQ는 손님/하우스맨 **탭 분리**. ① **`faq.js` — 52문항**(손님 33 · 하우스맨 19), 각각 8개 묶음. 손님: 입·퇴실 / 요금 / 객실 비품 / 오션월드 / 스키월드 / 반려동물 / 시설 위치 / 분실물. 하우스맨: 객실 상태 / 층별 비품 / 주의 / 카드키 / 린넨실 / 단체 / 응대 원칙 / 평가. **답은 전부 앱에 등록된 자료(공식홈·현장 업무카드)에서만** 가져왔고 추측으로 쓴 답은 없다. ② **고객 안내문 변환**: 손님 문항마다 `say` 필드 — 내부 용어(구배불량·불출 등)와 판단을 빼고 그대로 읽어 줄 수 있는 문장. 복사 버튼(클립보드 API 실패 시 execCommand 폴백). 응대 원칙(요금은 판단하지 않고 이관)이 문장에 반영돼 있다. ③ **객실 상태 보드**(`panels.js`): 동·층별 재실/공실/외출/고장 집계 + 호수 칸. 칸을 누르면 미확인→공실→재실→외출→고장 순환, `Store.setRoomStatus` → `applyChanges` 경로라 **감사 로그와 Undo가 그대로 적용**된다. 검증: 3개 동 95실 18개 층, 순환·기록 확인. ④ **요금 계산기**: 1박료 + 인원 추가(11,000 / **소노캄 50,000**) + 침구 추가(25,000, 소노캄은 미청구) + 조기입실 4구간 + 취소 위약금 6구간. 검증: 2인 추가 222,000 → 소노캄 체크 시 300,000. 화면에 **'참고용이며 확정은 프런트·1588-4888'** 명시. ⑤ 스튜디오에 카드 3장으로 진입. `Panels`는 Drill과 같은 껍데기를 쓰되 단계는 없다.
- 2026-08-01: **v0.15.0 시설 성격별 화면 분기**. 지적: *모든 구역을 일정하게 다루고 나열식이다. 지하 1층뿐인 비바 플렉스 몰에 층 정보가 왜 필요하냐.* → `layout` 필드를 두고 루트 화면을 9갈래로 갈랐다. **resort**(A·B·C·캄) 층 정면→객실 평면 유지 / **hotel**(D) 핵심 수치 3칸 + '리조트와 다른 점'(칫솔·치약·미니바) 먼저 / **pet**(E) 반려동물 규정을 최상단 경고 목록으로 / **estate**(펠리체·빌리지) 취사·미취사 **동 구분 배지** + 등급별 카드 + 전망 층 / **waterpark**(오션월드) '꼭 챙겨야 할 것(✓)' vs '안 되는 것(✕)' 대비 + 대여 요금표 / **ski**(메인센터·슬로프·스노위랜드) **이용 순서 1·2·3** 먼저 / **mall**(비바 플렉스 몰) **층 단계 제거** — `oneFloor: '지하 1층'`을 명시하고 아이콘 카테고리 그리드로 바로 / **dining** 업장 카드 / **help** 한 장 정보 카드. 검증: 오션월드 ✓2·✕4, 소노펫 경고 4, 펠리체 동 배지, D동 수치 3칸+✓3, 메인센터 순서 3단계, 비바 플렉스 몰 층 0·카테고리 7.
- 2026-08-01: **v0.14.0 시작 화면 + 앱 아이콘 새로 제작**. ① **스플래시 신설**: 갈색 방사형 그라데이션 위에 마크가 아래에서 올라오며(`spIn`/`spRise`) 눈덮인 봉우리가 페이드인, 능선이 `stroke-dasharray`로 그려지고(`spDraw`), 이름·부제가 8px 올라오며 등장, 하단에 얇은 진행 바(무한 슬라이드). 전부 오버슈트 없는 ease-out, `prefers-reduced-motion`이면 모션 제거. **최소 노출 900ms** 보장(부팅이 빨라도 깜빡이지 않게), 이후 0.42s 페이드+살짝 확대되며 사라지고 DOM에서 제거. ② **갇힘 방지 2중 안전장치**: `app.js`의 4초 타이머 + **index.html 인라인 5초 타이머**. app.js가 로드·파싱에 실패해도 시작 화면이 반드시 걷힌다(인라인이라 외부 스크립트에 의존하지 않음). ③ **앱 아이콘 재제작**: 기존 v0.5 teal→clay 아이콘을 버리고 현재 브랜드(갈색 배경 + 초록→라임 봉우리 + 흰 설면 + 베이지 능선)로 192·512 재생성. **maskable 전용 아이콘 별도 생성**(라운드 없이 배경을 꽉 채우고 안전영역 안으로 도형 축소) — 안드로이드에서 아이콘이 잘리던 위험 제거. manifest에 `any`/`maskable` 분리 등록, `background_color`를 스플래시와 같은 갈색(#7a4a22)으로 맞춤. iOS용 `apple-mobile-web-app-*` 메타 추가. **검증 한계**: 브라우저 패널이 합성을 멈춘 상태라 애니메이션 '진행'은 시간 샘플링으로 확인하지 못했다. 구조·적용된 애니메이션 이름·전체화면 여부·색은 확인함.
- 2026-08-01: **v0.13.0 지도를 평면 배치도로 · 모든 층에 평면도**. ① **지도 = 위에서 내려다본 배치도(site plan)**. 입체 블록(측면 + 상단 하이라이트 + 접지 그림자)을 걷어내고 **건물 외곽선 + 지붕 능선(ridge)·귀마루(hip) + 코어 표시**로 다시 그렸다. 도로는 흰 노면 + 파선 중앙선, 부지는 파선 외곽의 녹지, 슬로프는 등고선 있는 설면, 우측 하단에 **방위표(N)**. 검증: 평면 요소 foot 25·ridge 23, 입체 요소 side 0·gloss 0. ② **층으로 들어가면 그 층 평면도**가 항상 나온다. 객실이 있는 층은 **객실 평면**(호수·상태색), 객실이 없는 층(창고·린넨·시설)은 **용도별 평면**(`planZones`) — 층 내용을 실 단위로 쪼개 외벽·복도·출입문·코어와 함께 그린다. 비바 플렉스 몰 B1은 10실로 표시된다. ③ 이전 문구('등록된 객실이 없습니다'로 끝나던 빈 화면)를 없애고, 객실 목록을 받으면 용도 평면 → 객실 평면으로 바뀐다는 안내로 교체.
- 2026-08-01: **v0.12.0 전 시설 드릴다운 확대 + 경쟁 조건 버그 2건**. ① **31곳 전부 들어갈 수 있게 됨**(이전엔 층 자료가 있는 7곳만). 루트 화면이 3갈래로 갈린다 — **층**(정면에서 층 선택, 7곳: 메인센터·비바플렉스몰·소노벨D·소노캄·파인C·오크B·소노펠리체), **구역**(층 자료가 없으면 `items`를 종류별 카드로, 9곳: 슬로프·스노위랜드·오션월드·야외별관·소노펫·소노벨A·펠리체빌리지·등산로·주차장), **미확보**(확인된 게 없으면 그렇게 표시 + 아는 내용만, 15곳). **층을 임의로 만들지 않았다** — 비바 플렉스 몰에서 지어냈다가 지적받은 원칙을 그대로 적용. 구역 카드는 전부 공식홈·가이드맵에서 확인된 `items`에서만 나온다. ② 확인된 사실 보강: 소노벨 A 객실 스펙(87.6㎡·침실2·화장실2·5~8인·취사/미취사 선택·파크뷰), 스노위랜드 접근 경로(메인센터 1층 입장권 → 2층 곤돌라), CC 3코스·골프연습장·승마클럽·BMW·셔틀버스 위치 메모. 이제 **빈 곳 0** (31/31에 최소 1개 이상 내용). ③ **버그 A — 닫자마자 다시 열면 빈 화면**: `close()`의 240ms 정리 타이머가 그 사이 새로 연 화면을 지웠다. `closeToken`으로 무효화. ④ **버그 B — 화면 누적**: 다시 열 때 `#dStage`를 비우지 않아 이전 건물의 `.dpage`가 계속 쌓였다(48개까지 확인). `open()`에서 즉시 비운다. 두 버그 모두 **31곳 연속 열기/닫기 테스트**로 재현하고 수정 확인(최대 페이지 1, 실패 0).
- 2026-08-01: **v0.11.1 평면도 제대로 · 비바 플렉스 몰 층 정정**. ① **비바 플렉스 몰은 지하 1층 한 층 규모**(사장님 확인). 앞서 3F/2F/1F/B1로 나눈 것은 **내가 지어낸 것** — 공식홈에 층 표기가 없어 임의 배분했고 잘못이었다. B1 한 층에 전부 있는 것으로 정정하고 경고 문구(`floorNote`)는 제거. **교훈: 확인 안 된 건 '미확보'로 비워 두고 임의 배분하지 않는다.** ② **평면도를 도면답게 다시 그림**. 이전에는 복도 위아래로 칩을 늘어놓은 수준이었다. 지금은 **두꺼운 외벽(5px) · 중앙 복도 · 객실 경계벽 · 복도 쪽 출입문(벽을 끊는 개구부 + 스윙 호) · 코어(엘리베이터·계단 / 린넨실·창고)** 를 갖춘 평면. 객실 칸 74×78, 복도 44. 상태색으로 채우고 호수·상태를 칸 안에 적는다. ③ 안내 문구도 정확히 — '실측 도면이 아니라 호수 순서로 그린 개념 평면이며 엘리베이터·린넨실 위치는 예시'임을 화면에 명시. **실제 평면 자료를 받으면 좌표를 그대로 반영할 수 있는 구조**(rooms 배열 순서 → 셀 위치).
- 2026-08-01: **v0.11.0 객실 상태 · 층 평면 · 시설별 문구**. ① **`rooms` 컬렉션 신설**(동기화·감사로그·Undo 대상): `{bld,no,floor,type,status,note}`. 상태는 **미확인·공실·재실·외출·고장** 5종. `ensureRooms()`가 RoomData의 확인된 객실을 멱등 등록(현재 95실). 객실 화면에서 상태 버튼을 누르면 `applyChanges` 단일 경로로 기록돼 되돌릴 수 있다. **프런트 재실 데이터가 연결되면 자동 반영되도록 자리를 만들어 둔 것** — 지금은 수동 표시. ② **층 평면 개념도**: 복도를 사이에 두고 호수 순서대로 위·아래로 배치, 칸마다 호수와 상태를 표시하고 상태색으로 구분(범례 포함). 칸 크기를 62×46px로 **고정**하고 좁은 화면에서는 평면만 가로 스크롤 — 처음엔 화면 폭에 맞춰 줄어들어 글씨가 5px가 되던 것을 고쳤다. ③ **시설 종류별 문구 분기(지적 반영)**: 숙박이 아닌 곳에 '린넨실·객실·전망'을 쓰지 않는다. `WORDS[kind]`로 숙박=린넨실·비품/객실 배치, 놀이·편의=시설, 식음=업장. 객실 평면과 '객실 기준' 섹션은 **숙박에서만** 나온다. ④ **비바 플렉스 몰 구역 구성** 추가(3F 노래방·PC방·오락실 / 2F 볼링·사격·당구·사우나 / 1F 푸드코트·식당 / B1 가족놀이·쇼핑·약국). **⚠ 층 배분은 미확인** — 공식홈에 시설 목록만 있고 층 표기가 없어 `floorNote`로 경고를 띄운다. ⑤ **지도 등장 애니메이션**: 블록이 y좌표 순서로 위에서부터 내려앉는다(`--d` 스태거, 0.5s). `prefers-reduced-motion`이면 비활성. ⑥ **비공개 실사 사진 통로**: `Store.privateFile()` / `roomPhoto()` — 비공개 저장소 `photos/{동}/{호수}.jpg`(없으면 `_default.jpg`)를 **팀 토큰으로** 받아 blob으로 표시. 공개 주소로는 나가지 않는다. 사진이 없으면 자체 일러스트 + 공식 페이지 링크로 폴백. **사용자에게 고지한 사실**: GitHub Pages 주소는 로그인 없이 열리므로 `docs/`에 넣는 파일은 전부 공개된다. 그래서 저작권 있는 사진은 공개 저장소가 아니라 **비공개 저장소 경로**로만 다룬다. **남은 확인**: 비바 플렉스 몰 층 배분, 층별 호수 전체 목록(현재는 확인된 95실만).
- 2026-08-01: **v0.10.0 드릴다운 — 건물 → 층 → 객실 → 실사**. ① **`drill.js` 신설**: 전체화면 오버레이 4단계. (1) **건물 정면 일러스트**에서 층 클릭 — 실사 특징을 따라 그린 입면(소노벨=크림 벽+정중앙 진초록 세로 띠+옥상 사인, 소노캄/펫=베이지 벽+갈색 박공지붕, 펠리체=청록 지붕), 층 오른쪽에 **신경 쓸 객실 수 배지**. (2) **층 상세** — 그 층의 린넨실·비품(MapData.stack), 객실 기준(취사/세미취사/클린/펫, 전자레인지·컴퓨터), **실시간 하자·습득물**, 객실 배치 그리드. (3) **객실** — 호수·층·타입·특이사항 태그·상태(하자 진행/습득물 보관/이상 없음)와 진행 중 하자·습득물 목록. (4) **실사** — 객실 구성 표 + 공식 페이지 열기. ② **`roomdata.js` 신설 — 객실 단위 데이터**: 현장 게시물·업무 카드에서 **확인된 객실만** 등록(추측 금지). 캄 구배불량 43실·지정객실 1804/1806·지퍼불량 1214/1220/1510·속커튼X 1312/713/1621·전기포트 메모 18실, 파인(C) 2F 24실(일룸/원목/223 더블침대), 오크(B) 지정객실 2002/2035. 호수 앞자리로 층을 판별한다. 자료 없는 층은 **'등록된 객실 정보가 없습니다'**로 비워 두고 채우지 않는다. ③ **애니메이션**: 단계 전진은 오른쪽에서 슬라이드-인, 뒤로가기는 반대 방향. 280ms `--ease`, 이동은 26px로 절제. 브레드크럼으로 임의 단계 복귀 가능. `prefers-reduced-motion`이면 이동 없이 페이드만. ④ **버그 예방**: 페이지 등장 클래스를 `requestAnimationFrame`이 아니라 `setTimeout(10)`으로 건다 — 화면이 합성되지 않는 상황(백그라운드 탭 등)에서 rAF가 지연돼 화면이 영영 안 보이는 문제. 이 저장소에서 예전에 바텀시트로 같은 버그를 겪었다. ⑤ **객실 실사 사진은 앱에 싣지 않는다** — 소노 저작물. 대신 각 동의 공식 객실 페이지(`roomsUrl`)를 새 창으로 연다. 실사 전 단계는 전부 자체 일러스트. **한계**: 층별 자료가 없는 동(A·소노펫)은 정면 대신 안내 문구가 나온다. 소노펠리체는 전망 기준 층(1~6F / 7~17F)만 있어 2단계까지만 유효.
