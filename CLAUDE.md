...
- 사진은 **비공개 저장소에만** 둔다 — 시설 사진 `places/{placeId}/`, 객실 사진 `photos/{동}/{호수}.jpg`. 공개 `docs/`에는 절대 두지 않는다(팀 코드로 연결된 기기만 토큰으로 받아감).
- **큰 블록을 통으로 교체할 때는 교체 전후의 함수 목록을 비교할 것.** v0.15.0에서 이걸 안 해 `facadeView` 등 4개가 사라져 A·B·C·캄동이 두 버전 동안 깨져 있었다.
